...
# DB.run "CREATE TABLE program (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, code VARCHAR(255) NOT NULL, name VARCHAR(255) NOT NULL, university_id INTEGER NOT NULL)"
@programs = DB[:program]
	...
	each{|university|
		...
		if result.include? 'Programa:'
			...
			programs.each{|program|
				# debugger
				bar.increment!
				@programs.insert(:code=> program[0].to_i, :name => program[1], :university_id => code)
			}
